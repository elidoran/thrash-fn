...
const critical = require('./criticals-table.js')
...
module.exports = function calculate(result) {
  ...
  // update these result values based on other result values.
  ...
  // eventually, use the above calculations to determine we are
  // "certain enough" to stop.
  // Maybe track how much the results change from interval to interval
  // and stop when the change is very small? (when deviation is repetitive?)
...
function variance(result) {
  ...
  for (const time of result.times) {
    variance += Math.pow((time - result.average), 2)
  }
